...
func CheckBasicAuth(r *http.Request) error {
	bu := os.Getenv("BASIC_AUTH_USER")
	bp := os.Getenv("BASIC_AUTH_PASSWORD")
	u, p, ok := r.BasicAuth()
	if ok == false {
		return errors.New("Failed with basic auth")
	}
	if u != bu || p != bp {
		return errors.New("Failed with basic auth. user or password not matched")
	}
	return nil
}
